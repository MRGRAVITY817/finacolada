...
mod test {
    ...
    fn extract_cashflow_statement_data() {
        ...
        // Assert
        assert_snapshot!(result.get_column_names().join(", "), @"현금및현금성자산의증가, 금융자산처분손실, 만기보유금융자산의감소, 주식보상비환입, 종업원급여, 매도가능금융자산의감소, 사채의증가, 사채상환손실, 현금유출이없는비용등가산, 차입금의감소, 금융자산평가이익, 재고자산감모손실환입, 기타수익, 미지급금의증가, 주식매입선택권의행사, 기타부채의감소, 사채의감소, 법인세납부(-), 금융자산처분이익, 기타운전자본의변동, 금융자산손상차손환입, 이자지급(-), 만기보유금융자산의증가, 외환손실, 기타재무활동으로인한현금유입액, 재고자산폐기(처분)손실, *영업에서창출된현금흐름, 투자활동으로인한현금흐름, (재무활동으로인한현금유출액), 자산손상차손환입, 반품(환불)부채전입액, 반품(환불)부채환입액, 기타투자활동으로인한현금흐름, 자기주식의취득, 기초현금및현금성자산, 금융부채관련이익, 배당금지급, 배출부채환입액, 이자비용, 유상증자, 당기손익-공정가치측정 금융자산관련이익, 정부보조금등의변동, 자산재평가이익, 종속회사관련이익, 배출부채전입액, 장기금융상품의증가, 충당부채전입액, 재무활동으로인한현금유입액, 부채의증가(감소), 금융부채관련손실, 퇴직급여, 유동성장기부채의증가, 기타비용, 차입금의증가, 감가상각비, 연결범위변동으로인한현금의증가, 충당부채환입액, 법인세환입, 지분법관련손실, 계약부채전입액, 매출채권처분손실, 자기주식의처분, 장기금융상품의감소, 이자수입, 대손충당금환입액, 재고자산감모손실, 금융자산평가손실, 자본구성항목의증가, (현금유입이없는수익등차감), 매출채권처분이익, 자본구성항목의감소, 당기순손익, 투자자산평가이익, 금융자산손상차손, 자산의감소(증가), 퇴직급여충당부채환입액, 배당금수입, 장기대여금의감소, 무형자산의감소, 투자활동으로인한현금유입액, 생물자산의감소, 매도가능금융자산의증가, 지분법관련이익, 투자부동산의증가, 파생상품이익, 기타영업활동으로인한현금흐름, 관계기업등지분관련투자자산의감소, 유상감자, 파생상품의변동, 법인세비용차감전계속사업이익, 대손상각비, 금융원가, 재고자산폐기(처분)이익, 금융수익, 당기손익-공정가치측정 금융자산관련손실, 중단영업관련현금흐름, 기타의대손상각비, 자산재평가손실, 자산손상차손, 기타금융부채의감소, 기타재무활동으로인한현금흐름, 투자부동산의감소, 기타재무활동으로인한현금유출액, 사채상환이익, 기말현금및현금성자산, 법인세비용, 투자자산평가손실, 주식보상비, 이자수익, 유동성장기부채의감소, 자산처분(폐기)손실, 관계기업등지분관련투자자산의증가, 기타부채의증가, 계약부채환입액, 영업활동으로인한자산부채변동(운전자본변동), 유형자산의증가, 기타비유동금융자산의증가, 유동금융자산의증가, 기타투자활동으로인한현금유출액, 자산처분(폐기)이익, 장기대여금의증가, 파생상품손실, 미지급금의감소, 종속기업관련손실, 기타비유동금융자산의감소, 재무활동으로인한현금흐름, 관계기업관련손익, 배당금지급(-), 법인세수익, 생물자산의증가, 배당금수익, 외환이익, 무형자산상각비, 유형자산의감소, 유동금융자산의감소, (투자활동으로인한현금유출액), 기타금융부채의증가, 영업투자재무활동기타현금흐름, 무형자산의증가, 환율변동효과, 기타투자활동으로인한현금유입액, 영업활동으로인한현금흐름, 연도");
    }
    ...
    fn extract_comprehensive_income_statement_data() {
        ...
        let result = get_statement_table(input).unwrap();
        // Assert
        assert_snapshot!(result.get_column_names().join(", "), @"계속영업이익, 이자비용, 판매비와관리비, 대손상각비, 영업이익, 매출원가, 대손충당금환입, 매출채권처분손실, 외환이익, 자산처분(폐기)이익, 영업이익(발표기준), 금융자산평가손실, 법인세비용, 인건비, 금융자산처분손실, 배당금수익, 로열티수익, 재고자산감모손실, 재고자산폐기(처분)손실, 금융자산손상차손환입, 유무형자산상각비, 재고자산폐기(처분)이익, 자산평가이익, 금융자산처분이익, 기타대손상각비, 당기손익-공정가치측정 금융자산관련손실, 자산손상차손, 금융수익, 대손충당금환입액, 지배주주순이익, 기타금융원가, 매출총이익, 기타, 금융원가, 연구개발비, 이자수익, 세전계속사업이익, 중단영업이익, 재고자산감모손실환입, 기타비용, 종속기업,공동지배기업및관계기업투자주식처분손익, 종속기업,공동지배기업및관계기업관련손익, 자산처분(폐기)손실, 종속기업,공동지배기업및관계기업투자주식손상관련손익, 자산손상차손환입, 관리비, 당기손익-공정가치측정 금융자산평가손실, 충당부채전입액, 기타수익, 충당부채환입액, 광고선전비, 금융자산평가이익, 기타원가성비용, 매출액, 자산평가손실, 매출채권처분이익, 당기손익-공정가치측정 금융자산평가이익, 당기손익-공정가치측정 금융자산관련이익, 비지배주주순이익, 지분법손익, 당기순이익, 판매비, 수수료수익, 금융자산손상차손, 파생상품이익, 기타금융수익, 외환손실, 임대료수익, 파생상품손실, 연도");
    }
    ...
    fn extract_financial_position_statement_data() {
        ...
        let result = get_statement_table(input).unwrap();
        // Assert
        assert_snapshot!(result.get_column_names().join(", "), @"비유동자산, 장기차입금, 장기매입채무및기타비유동채무, 비유동종업원급여충당부채, 장기당기법인세부채, 투자부동산, 기타자본, 비지배주주지분, 기타비유동부채, 계약자산, 유동자산, 유동생물자산, 비유동부채, 기타비유동자산, 반품(환불)자산, 사채, 신종자본증권, 장기당기법인세자산, 유동부채, 매각예정으로분류된처분자산집단에포함된부채, 매각예정비유동자산및처분자산집단, 유동금융자산, 자본, 비유동금융부채, 기타유동자산, 관계기업등지분관련투자자산, 이연법인세부채, 기타금융업부채, 반품(환불)부채, 자본잉여금, 배출권, 유형자산, 현금및현금성자산, 자본금, 자산, 단기차입금, 당기법인세부채, 배출부채, 매출채권및기타유동채권, 이익잉여금(결손금), 당기법인세자산, 매입채무및기타유동채무, 비유동생물자산, 기타유동부채, 기타장기충당부채, 무형자산, 유동종업원급여충당부채, 단기사채, 계약부채, 재고자산, 이연법인세자산, 지배기업주주지분, 기타포괄손익누계액, 유동성장기부채, 장기금융자산, 장기매출채권및기타비유동채권, 기타금융업자산, 유동금융부채, 부채, 기타단기충당부채, 연도");
    }
    ...
    async fn latest_samsung_stock_price() {
        ...
        assert_snapshot!(result.0.to_string(), @"61900");
    }
    ...
    async fn latest_metrics_for_samsung() {
        ...
        assert_snapshot!( format!("{:?}", result), @"Metrics { per: 9.417314, pcr: 5.6762953, pbr: 1.2119907, psr: 1.321633 }");
    }
}
