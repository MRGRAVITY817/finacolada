...
mod test {
    ...
    fn extract_cashflow_statement_data() {
        ...
        // Assert
        assert_snapshot!(result.get_column_names().join(", "), @"기타재무활동으로인한현금흐름, (재무활동으로인한현금유출액), 법인세비용, 정부보조금등의변동, 계약부채환입액, 사채상환이익, 사채상환손실, 자산재평가손실, 미지급금의증가, 금융자산손상차손, 금융자산처분손실, 무형자산상각비, 기초현금및현금성자산, 퇴직급여충당부채환입액, 자산재평가이익, 유동금융자산의감소, 기타금융부채의증가, 배당금지급, 투자자산평가이익, 지분법관련이익, 유상감자, 외환손실, 투자활동으로인한현금흐름, 만기보유금융자산의증가, 기타투자활동으로인한현금유입액, 배당금수익, 관계기업등지분관련투자자산의증가, 재무활동으로인한현금유입액, 사채의감소, 투자자산평가손실, 관계기업관련손익, 자기주식의취득, 재고자산폐기(처분)손실, 계약부채전입액, 주식보상비, 종속기업관련손실, 장기금융상품의증가, 외환이익, 자산처분(폐기)이익, 매도가능금융자산의감소, 재고자산감모손실, 자산손상차손환입, 기타재무활동으로인한현금유출액, 법인세납부(-), 이자수입, 사채의증가, 관계기업등지분관련투자자산의감소, 재고자산감모손실환입, 이자지급(-), 자본구성항목의감소, 현금및현금성자산의증가, 충당부채전입액, 생물자산의감소, 기타금융부채의감소, 이자수익, 만기보유금융자산의감소, 법인세비용차감전계속사업이익, 감가상각비, 매출채권처분이익, 반품(환불)부채환입액, 법인세환입, 충당부채환입액, 자산손상차손, 유동금융자산의증가, 부채의증가(감소), 유형자산의증가, 매도가능금융자산의증가, 무형자산의증가, 배출부채전입액, 금융원가, 무형자산의감소, 영업투자재무활동기타현금흐름, 금융부채관련이익, 기타비유동금융자산의감소, 환율변동효과, 금융자산손상차손환입, 기타부채의감소, 기말현금및현금성자산, 차입금의증가, 당기순손익, 재고자산폐기(처분)이익, 기타운전자본의변동, 당기손익-공정가치측정 금융자산관련이익, *영업에서창출된현금흐름, 투자활동으로인한현금유입액, 기타재무활동으로인한현금유입액, 기타수익, 파생상품의변동, 금융자산평가이익, 파생상품손실, 배출부채환입액, 종속회사관련이익, 법인세수익, 투자부동산의증가, 지분법관련손실, 퇴직급여, 자산의감소(증가), 주식보상비환입, 차입금의감소, 기타영업활동으로인한현금흐름, 주식매입선택권의행사, 연결범위변동으로인한현금의증가, 기타의대손상각비, 종업원급여, 기타부채의증가, 재무활동으로인한현금흐름, 자기주식의처분, 유상증자, 대손충당금환입액, (투자활동으로인한현금유출액), 자산처분(폐기)손실, 이자비용, 기타비용, 파생상품이익, 배당금수입, (현금유입이없는수익등차감), 기타비유동금융자산의증가, 생물자산의증가, 유동성장기부채의증가, 장기금융상품의감소, 중단영업관련현금흐름, 금융자산평가손실, 배당금지급(-), 자본구성항목의증가, 영업활동으로인한현금흐름, 금융부채관련손실, 투자부동산의감소, 유동성장기부채의감소, 유형자산의감소, 금융자산처분이익, 장기대여금의감소, 기타투자활동으로인한현금흐름, 반품(환불)부채전입액, 기타투자활동으로인한현금유출액, 금융수익, 영업활동으로인한자산부채변동(운전자본변동), 장기대여금의증가, 현금유출이없는비용등가산, 매출채권처분손실, 당기손익-공정가치측정 금융자산관련손실, 미지급금의감소, 대손상각비, 연도");
    }
    ...
    fn extract_comprehensive_income_statement_data() {
        ...
        let result = get_statement_table(input).unwrap();
        // Assert
        assert_snapshot!(result.get_column_names().join(", "), @"자산손상차손환입, 자산평가손실, 인건비, 기타, 금융자산손상차손환입, 외환이익, 파생상품손실, 판매비와관리비, 당기손익-공정가치측정 금융자산평가이익, 매출총이익, 배당금수익, 재고자산폐기(처분)손실, 판매비, 당기손익-공정가치측정 금융자산관련이익, 대손충당금환입, 재고자산감모손실환입, 종속기업,공동지배기업및관계기업관련손익, 영업이익(발표기준), 계속영업이익, 재고자산폐기(처분)이익, 매출채권처분손실, 매출액, 광고선전비, 충당부채환입액, 수수료수익, 종속기업,공동지배기업및관계기업투자주식손상관련손익, 금융자산평가이익, 기타비용, 대손충당금환입액, 금융자산처분이익, 재고자산감모손실, 외환손실, 자산처분(폐기)이익, 이자수익, 세전계속사업이익, 자산평가이익, 영업이익, 기타대손상각비, 매출원가, 임대료수익, 종속기업,공동지배기업및관계기업투자주식처분손익, 유무형자산상각비, 파생상품이익, 충당부채전입액, 금융수익, 금융자산평가손실, 중단영업이익, 당기순이익, 기타원가성비용, 금융자산손상차손, 기타금융원가, 법인세비용, 이자비용, 연구개발비, 지배주주순이익, 지분법손익, 자산처분(폐기)손실, 당기손익-공정가치측정 금융자산평가손실, 금융자산처분손실, 로열티수익, 대손상각비, 관리비, 금융원가, 기타수익, 당기손익-공정가치측정 금융자산관련손실, 비지배주주순이익, 매출채권처분이익, 기타금융수익, 자산손상차손, 연도");
    }
    ...
    fn extract_financial_position_statement_data() {
        ...
        let result = get_statement_table(input).unwrap();
        // Assert
        assert_snapshot!(result.get_column_names().join(", "), @"부채, 기타비유동부채, 유동금융자산, 자본, 자본금, 유형자산, 매각예정비유동자산및처분자산집단, 당기법인세부채, 재고자산, 기타유동부채, 매각예정으로분류된처분자산집단에포함된부채, 계약자산, 장기매입채무및기타비유동채무, 기타금융업부채, 이익잉여금(결손금), 단기차입금, 무형자산, 비유동부채, 비유동금융부채, 투자부동산, 이연법인세자산, 배출권, 당기법인세자산, 비유동생물자산, 유동성장기부채, 매출채권및기타유동채권, 관계기업등지분관련투자자산, 이연법인세부채, 유동자산, 장기당기법인세부채, 장기당기법인세자산, 기타유동자산, 계약부채, 기타금융업자산, 자산, 유동금융부채, 비유동자산, 유동부채, 매입채무및기타유동채무, 지배기업주주지분, 기타자본, 장기금융자산, 기타포괄손익누계액, 신종자본증권, 사채, 비유동종업원급여충당부채, 유동생물자산, 기타장기충당부채, 반품(환불)자산, 현금및현금성자산, 자본잉여금, 장기차입금, 단기사채, 비지배주주지분, 기타비유동자산, 장기매출채권및기타비유동채권, 기타단기충당부채, 유동종업원급여충당부채, 배출부채, 반품(환불)부채, 연도");
    }
    ...
    async fn latest_samsung_stock_price() {
        ...
        assert_snapshot!(result.0.to_string(), @"62900");
    }
    ...
    async fn latest_metrics_for_samsung() {
        ...
        assert_snapshot!( format!("{:?}", result), @"Metrics { per: 9.56945, pcr: 5.7679963, pbr: 1.2315705, psr: 1.3429841 }");
    }
}
