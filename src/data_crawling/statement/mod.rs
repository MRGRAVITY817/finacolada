...
mod test {
    ...
    fn extract_cashflow_statement_data() {
        ...
        // Assert
        assert_snapshot!(result.get_column_names().join(", "), @"자기주식의처분, 영업활동으로인한자산부채변동(운전자본변동), 유동성장기부채의감소, 금융부채관련이익, 기타영업활동으로인한현금흐름, 자산의감소(증가), 금융자산손상차손환입, 무형자산의증가, 기말현금및현금성자산, 기타금융부채의감소, 충당부채환입액, 관계기업등지분관련투자자산의증가, (재무활동으로인한현금유출액), 파생상품손실, *영업에서창출된현금흐름, 이자수익, 기타투자활동으로인한현금유출액, 이자비용, 무형자산상각비, 사채의증가, 매도가능금융자산의감소, 유동성장기부채의증가, 자기주식의취득, 금융수익, 법인세비용차감전계속사업이익, 유동금융자산의증가, (현금유입이없는수익등차감), 금융자산손상차손, 매출채권처분손실, 금융원가, 외환손실, 기타비용, 자산손상차손환입, 배당금수입, 관계기업등지분관련투자자산의감소, 기타투자활동으로인한현금흐름, 연결범위변동으로인한현금의증가, 금융부채관련손실, 금융자산처분손실, 생물자산의감소, 차입금의감소, 자산처분(폐기)이익, 충당부채전입액, 영업활동으로인한현금흐름, 기타비유동금융자산의증가, 관계기업관련손익, 배당금수익, 부채의증가(감소), 재무활동으로인한현금유입액, 자산손상차손, 감가상각비, 생물자산의증가, 계약부채환입액, 유상감자, 정부보조금등의변동, 현금및현금성자산의증가, 자본구성항목의증가, 기초현금및현금성자산, 현금유출이없는비용등가산, 계약부채전입액, 재고자산폐기(처분)손실, 주식보상비, 투자활동으로인한현금유입액, 대손상각비, 배출부채환입액, 재고자산감모손실환입, 유동금융자산의감소, 주식보상비환입, 이자수입, 재고자산감모손실, 퇴직급여, 금융자산평가이익, 투자활동으로인한현금흐름, 법인세납부(-), 재고자산폐기(처분)이익, 자산재평가이익, 장기대여금의감소, 파생상품의변동, 차입금의증가, 미지급금의증가, 장기금융상품의감소, 배당금지급, 자본구성항목의감소, 대손충당금환입액, 퇴직급여충당부채환입액, 반품(환불)부채전입액, 기타비유동금융자산의감소, 법인세비용, 법인세수익, 기타운전자본의변동, 기타투자활동으로인한현금유입액, 유상증자, 당기손익-공정가치측정 금융자산관련손실, 만기보유금융자산의증가, 유형자산의증가, 기타부채의감소, 배출부채전입액, 기타의대손상각비, 지분법관련손실, 기타부채의증가, 환율변동효과, 종업원급여, 이자지급(-), 미지급금의감소, 장기금융상품의증가, (투자활동으로인한현금유출액), 매도가능금융자산의증가, 자산재평가손실, 기타금융부채의증가, 기타재무활동으로인한현금유입액, 당기순손익, 투자자산평가손실, 매출채권처분이익, 반품(환불)부채환입액, 투자자산평가이익, 법인세환입, 투자부동산의증가, 사채의감소, 만기보유금융자산의감소, 기타재무활동으로인한현금유출액, 무형자산의감소, 금융자산처분이익, 주식매입선택권의행사, 영업투자재무활동기타현금흐름, 금융자산평가손실, 당기손익-공정가치측정 금융자산관련이익, 투자부동산의감소, 종속기업관련손실, 자산처분(폐기)손실, 외환이익, 종속회사관련이익, 기타수익, 배당금지급(-), 유형자산의감소, 장기대여금의증가, 파생상품이익, 기타재무활동으로인한현금흐름, 사채상환손실, 재무활동으로인한현금흐름, 지분법관련이익, 중단영업관련현금흐름, 사채상환이익, 연도");
    }
    ...
    fn extract_comprehensive_income_statement_data() {
        ...
        let result = get_statement_table(input).unwrap();
        // Assert
        assert_snapshot!(result.get_column_names().join(", "), @"연구개발비, 파생상품이익, 영업이익(발표기준), 지분법손익, 이자수익, 이자비용, 자산손상차손환입, 판매비, 재고자산감모손실환입, 금융수익, 금융자산처분이익, 금융원가, 광고선전비, 금융자산손상차손, 종속기업,공동지배기업및관계기업투자주식처분손익, 매출채권처분이익, 재고자산폐기(처분)손실, 자산처분(폐기)이익, 종속기업,공동지배기업및관계기업투자주식손상관련손익, 외환손실, 세전계속사업이익, 충당부채전입액, 당기손익-공정가치측정 금융자산관련손실, 유무형자산상각비, 재고자산폐기(처분)이익, 임대료수익, 대손충당금환입, 자산처분(폐기)손실, 계속영업이익, 기타수익, 법인세비용, 중단영업이익, 자산손상차손, 대손충당금환입액, 당기손익-공정가치측정 금융자산평가손실, 당기손익-공정가치측정 금융자산관련이익, 지배주주순이익, 판매비와관리비, 수수료수익, 당기순이익, 매출액, 영업이익, 비지배주주순이익, 기타대손상각비, 파생상품손실, 금융자산처분손실, 기타금융수익, 재고자산감모손실, 당기손익-공정가치측정 금융자산평가이익, 로열티수익, 자산평가손실, 금융자산평가손실, 기타원가성비용, 기타, 대손상각비, 매출원가, 금융자산손상차손환입, 금융자산평가이익, 외환이익, 자산평가이익, 매출채권처분손실, 기타금융원가, 관리비, 기타비용, 배당금수익, 종속기업,공동지배기업및관계기업관련손익, 충당부채환입액, 인건비, 매출총이익, 연도");
    }
    ...
    fn extract_financial_position_statement_data() {
        ...
        let result = get_statement_table(input).unwrap();
        // Assert
        assert_snapshot!(result.get_column_names().join(", "), @"배출권, 기타장기충당부채, 비유동자산, 관계기업등지분관련투자자산, 장기금융자산, 장기당기법인세자산, 유동성장기부채, 계약부채, 단기차입금, 사채, 신종자본증권, 유동금융부채, 배출부채, 매각예정으로분류된처분자산집단에포함된부채, 기타금융업자산, 반품(환불)자산, 유동자산, 장기당기법인세부채, 당기법인세부채, 반품(환불)부채, 비지배주주지분, 유형자산, 당기법인세자산, 이연법인세자산, 유동금융자산, 매출채권및기타유동채권, 장기매출채권및기타비유동채권, 부채, 유동부채, 기타자본, 무형자산, 현금및현금성자산, 장기차입금, 자본잉여금, 계약자산, 비유동종업원급여충당부채, 매입채무및기타유동채무, 유동생물자산, 기타비유동자산, 기타단기충당부채, 유동종업원급여충당부채, 기타유동부채, 이연법인세부채, 비유동금융부채, 기타금융업부채, 지배기업주주지분, 이익잉여금(결손금), 비유동부채, 기타유동자산, 자산, 단기사채, 투자부동산, 자본금, 자본, 기타포괄손익누계액, 장기매입채무및기타비유동채무, 재고자산, 기타비유동부채, 매각예정비유동자산및처분자산집단, 비유동생물자산, 연도");
    }
    ...
    async fn latest_samsung_stock_price() {
        ...
        assert_snapshot!(result.0.to_string(), @"63100");
    }
    ...
    async fn latest_metrics_for_samsung() {
        ...
        assert_snapshot!( format!("{:?}", result), @"Metrics { per: 9.599878, pcr: 5.7863364, pbr: 1.2354865, psr: 1.3472543 }");
    }
}
